...
import shutil
import os
from config import TRAIN_DIR, AUGMENTED_DATA_DIR, CLASS_NAMES, TARGET_IMAGES_PER_CLASS, VAL_DIR
...
def explore_dataset(data_dir):
    ...
    # Iterate over global CLASS_NAMES (which does NOT include 'unknown' initially)
...
def augment_class(class_name, original_images, original_dir, output_dir, target_count):
    ...
    Args:
        ...
        original_images (list): List of filenames for original images in this class.
        original_base_dir (str): The base directory where the original images are located (e.g., TRAIN_DIR).
        output_dir (str): The directory where augmented images will be saved (e.g., AUGMENTED_DATA_DIR).
        target_count (int): The desired number of images for this class.
    """
...
def main():
    ...
    for class_name in CLASS_NAMES: # Iterate over known classes
        aug_class_path = os.path.join(AUGMENTED_DATA_DIR, class_name)
        if os.path.exists(aug_class_path):
            shutil.rmtree(aug_class_path)
    ...
    for class_name in [cn for cn in CLASS_NAMES ]:
    ...
    visualize_samples(AUGMENTED_DATA_DIR) # Visualizes the augmented training set (including its unknown)
    plot_class_distribution(AUGMENTED_DATA_DIR) # Plots distribution for augmented training set

    visualize_samples(VAL_DIR) # Visualizes the validation set (original known + generated unknown)
    plot_class_distribution(VAL_DIR) # Plots distribution for validation set
